...
describe("OLX.ba Search - Mixed input", function () {
  this.timeout(30000);

  it("Should handle search with mixed alphanumeric input", async function () {
    ...
    await searchInput.sendKeys("audi 2020", Key.ENTER);

    const currentUrl = await driver.getCurrentUrl();
    assert.ok(
      currentUrl.includes("olx.ba"),
      "Application crashed or navigated away on mixed input search"
    );
  });
});
