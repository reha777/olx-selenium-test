...
describe("OLX.ba Search - Numeric input", function () {
  this.timeout(30000);

  it("Should handle numeric search input", async function () {
    ...
    await searchInput.sendKeys("2020", Key.ENTER);

    const currentUrl = await driver.getCurrentUrl();
    assert.ok(
      currentUrl.includes("olx.ba"),
      "Application crashed or navigated away on numeric search"
    );
  });
});
